...
class QuestionBloc extends Bloc {
  ...
  Question get question => questionsRepository.question;
  ...
  Category? get category => categoriesRepository.get(question.categoryId ?? '');
...
class QuestionPage extends UI {
  ...
  Widget build(BuildContext context) {
    ...
    final isMcq = question.type == QuestionType.mcqMultiple ||
    ...
    return FScaffold(
      header: FHeader.nested(
        prefixActions: [
          FHeaderAction.back(onPress: _question.back),
        ],
        suffixActions: [
          ...
          if (isMcq)
            Column(
              children: [
                ...
                Column(
                  children: _question.question.options
                      .mapIndexed(
                        (index, option) => Column(
                          children: [
                            FTextField(
                              ...
                              initialValue: option,
                              onChange: (value) =>
                                  _question.question.options[index] = value,
                            ),
                            FHeaderAction(
                  ...
                  children: _question.question.options
                      .mapIndexed(
                        (index, option) => FTile(
                          title: option.text(),
                          enabled:
                              _question.question.correctAnswers.contains(index),
                          onPress: () {
                            if (question.type == QuestionType.bestChoice) {
                              _question.question.correctAnswers = [index];
                            } else {
                              if (_question.question.correctAnswers
                                  .contains(index)) {
